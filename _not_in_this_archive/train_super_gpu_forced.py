...
def main(cfg: DictConfig) -> None:
    ...
    if launcher == "stylegan":
        ...
        cmd = [
            ...
            f"++training.batch_size=8",  # OVERRIDE training.batch_size (++ prefix to override existing)
            f"++training.fp32=false",  # OVERRIDE training.fp32 (++ prefix to override existing)
            f"++training.nobench=false",  # OVERRIDE training.nobench (++ prefix to override existing)
            f"++training.allow_tf32=false",  # OVERRIDE training.allow_tf32 (++ prefix to override existing)
            f"++training.metrics=[fid50k_full]",  # OVERRIDE training.metrics (++ prefix to override existing)
            f"++training.seed=0",  # OVERRIDE training.seed (++ prefix to override existing)
            f"++training.data={dataset_path}",  # OVERRIDE training.data (++ prefix to override existing)
            f"++model.loss_kwargs.source=StyleGAN2Loss",  # OVERRIDE model.loss_kwargs.source (++ prefix to override existing)
            f"++model.loss_kwargs.style_mixing_prob=0.0",  # OVERRIDE model.loss_kwargs.style_mixing_prob (++ prefix to override existing)
            f"++model.discriminator.mbstd_group_size=4",  # OVERRIDE model.discriminator.mbstd_group_size (++ prefix to override existing)
            f"++model.discriminator.source=networks",  # OVERRIDE model.discriminator.source (++ prefix to override existing)
            f"++model.generator.source=networks",  # OVERRIDE model.generator.source (++ prefix to override existing)
            f"++model.generator.w_dim=512",  # OVERRIDE model.generator.w_dim (++ prefix to override existing)
            ...
            f"++training.num_workers=8",  # OVERRIDE training.num_workers (++ prefix to override existing)
            f"++training.subset=null",  # OVERRIDE training.subset (++ prefix to override existing)
            f"++training.mirror=true",  # OVERRIDE training.mirror (++ prefix to override existing)
            f"++training.cfg=auto",  # OVERRIDE training.cfg (++ prefix to override existing)
            f"++training.aug=ada",  # OVERRIDE training.aug (++ prefix to override existing)
            f"++training.p=null",  # OVERRIDE training.p (++ prefix to override existing)
            f"++training.target=0.6",  # OVERRIDE training.target (++ prefix to override existing)
            f"++training.augpipe=bgc",  # OVERRIDE training.augpipe (++ prefix to override existing)
            f"++training.freezed=0",  # OVERRIDE training.freezed (++ prefix to override existing)
            f"++training.dry_run=false",  # OVERRIDE training.dry_run (++ prefix to override existing)
            f"++training.cond=false",  # OVERRIDE training.cond (++ prefix to override existing)
